...
public class RequestLoggingFilter extends OncePerRequestFilter {
    ...
    private void logIncomingRequest(ContentCachingRequestWrapper request, String traceId) {
        ...
        log.info("[{}] --> {} {}{} from={} ua={}",
                traceId, request.getMethod(), request.getRequestURI(), queryString, clientIp, userAgent);
    }

    private void logRequestBody(ContentCachingRequestWrapper request, String traceId) {
        if (!isTextContentType(request.getContentType())) return;
    ...
    private Optional<String> parseTraceparent(String header) {
        ...
        if (traceId.length() != 32 || !traceId.matches("[0-9a-f]+")) return Optional.empty();
    ...
    private boolean isTextContentType(String contentType) {
        return Optional.ofNullable(contentType)
                .map(ct -> ct.startsWith("text/") || ct.contains("application/json")
                        || ct.contains("application/xml") || ct.contains("application/x-www-form-urlencoded"))
                .orElse(false);
    }
