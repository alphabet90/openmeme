...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const canonical = `/${locale}/memes/top${page > 0 ? `?page=${page}` : ""}`;
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: buildAlternates(`/memes/top${page > 0 ? `?page=${page}` : ""}`),
    },
    openGraph: {
...
export default async function TopMemesPage({ params, searchParams }: Props) {
  const { locale } = await params;
  const { page: pageRaw } = await searchParams;
  const page = parsePage(pageRaw);
  ...
  const breadcrumbs = [
    ...
    { name: t("breadcrumb_top"), href: "/memes/top" },
  ];
  ...
  const buildPageHref = (p: number) =>
    p > 0
      ? localePath(locale, `/memes/top?page=${p}`)
      : localePath(locale, "/memes/top");
