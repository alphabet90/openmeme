...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "nuevos" });
  const page = parsePage(pageRaw);
  const canonical = `/${locale}/memes/nuevos${page > 0 ? `?page=${page}` : ""}`;
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: buildAlternates(`/memes/nuevos${page > 0 ? `?page=${page}` : ""}`),
    },
    openGraph: {
...
export default async function NuevosMemesPage({ params, searchParams }: Props) {
  const { locale } = await params;
  const { page: pageRaw } = await searchParams;
  const page = parsePage(pageRaw);
  ...
  const [listing, categories, trending] = await Promise.all([
    getMemeListing({ sort: "created_at", page, limit: PAGE_SIZE, locale: apiLocale }).catch(() => null),
  ...
  const breadcrumbs = [
    ...
    { name: t("breadcrumb_nuevos"), href: "/memes/nuevos" },
  ];
  ...
  const buildPageHref = (p: number) =>
    p > 0
      ? localePath(locale, `/memes/nuevos?page=${p}`)
      : localePath(locale, "/memes/nuevos");
          ...
          <div className="container">
            ...
            <div className={styles.layout}>
              <div className={styles.primary}>
                <SectionTitle id="nuevos-title" icon={<span>⭐</span>}>
                  {t("section_title")}
                </SectionTitle>
